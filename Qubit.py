...
def oracle(m, x):
    oracle_qc = QuantumCircuit(256)
    target = QuantumCircuit(1)
    target.x(0)
    oracle_qc.append(target.to_instruction(), [i for i in range(256)])
    for i in range(256):
        oracle_qc.append(XGate().power(int(math.pow(2, i))), [i])
    for i in range(1, 256):
        oracle_qc.cx(i, 0)
    oracle_qc.x(0)
    oracle_qc.append(target.to_instruction().inverse(), [i for i in range(256)])
